...
draw_text(room_width * .5, 32, "Use of Scalar Example 3 - Room 28");
draw_set_font(fnt_reg);
draw_set_color(c_white)
draw_text(room_width * .5, 64, @"Add a escalation by adding easing");
...
length = draw_code(code_x, code_y, c_gray, "var new_speed = update_scalar(2, 8, scl_once, 0, 1, 0, 1); ");

//new_speed = new_speed * .88 + .12;
code_y += line_height * 2;
length = draw_code(code_x, code_y, c_gray, "new_speed = new_speed * .88 + .12;");

// new_speed = ease_scalar(eas_in, new_speed, 2);
code_y += line_height;
draw_code(code_x, code_y, c_green, "ease in with medium intensity so that flashing speeds up at end");
code_y += line_height;
length = draw_code(code_x, code_y, c_local_var, "new_speed ", c_white, " = ", c_user_funct, "ease_scalar ", c_white, "(", c_internal_var, "eas_in", c_white, ", ", c_local_var, "new_speed");
draw_code(code_x + length, code_y, c_white, ", ", c_internal_var, "2", c_white, ");");

//var new_a = update_scalar(0, new_speed, scl_bounce_repeat, 0, 0, 1, 1);
code_y += line_height * 2;
length = draw_code(code_x, code_y, c_gray, "var  new_a  =  update_scalar(0, ", c_local_var, "new_speed", c_gray, ", scl_bounce_repeat, 0, 0, 1, 1);");
code_y += line_height;

// new_a = ease_scalar(eas_in_out, new_a, 4);
draw_code(code_x, code_y, c_green, "ease in with high intensity so that image_alpha ramps during fade");
code_y += line_height;
length = draw_code(code_x, code_y, c_local_var, "new_a ", c_white, " = ", c_user_funct, "ease_scalar ", c_white, "(", c_internal_var, "eas_in", c_white, ", ", c_local_var, "new_a");
draw_code(code_x + length, code_y, c_white, ", ", c_internal_var, "4", c_white, ");");
code_y += line_height;
//	image_alpha = new_a;
draw_code (code_x, code_y, c_gray, "image_alpha = ", c_local_var, "new_a;");

code_y += line_height;

//var new_b = update_scalar(18, scl_once);
length = draw_code (code_x, code_y, c_gray, "var new_b = update_scalar(18, scl_once);");

// new_b = ease_scalar(eas_in, new_b, 2);
code_y += line_height;
draw_code(code_x, code_y, c_green, "ease in with high intensity so that image turns red");
code_y += line_height;
length = draw_code(code_x, code_y, c_local_var, "new_b ", c_white, " = ", c_user_funct, "ease_scalar ", c_white, "(", c_internal_var, "eas_in", c_white, ", ", c_local_var, "new_b");
draw_code(code_x + length, code_y, c_white, ", ", c_internal_var, "2", c_white, ");");
...
draw_code(code_x, code_y, c_gray, "var non_red_color = 255 - (", c_local_var, "new_b", c_gray, " * 255);;");

code_y += line_height

//var red_color = new_b * 105 + 150;
code_y += line_height;
length = draw_code(code_x, code_y, c_gray, "var red_color = ", c_local_var,"new_b", c_gray, " * 105 + 150;");
